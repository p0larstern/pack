...
func Build(logger logging.Logger, cfg config.Config, packClient PackClient) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: logError(logger, func(cmd *cobra.Command, args []string) error {
			...
			descriptor, err := parseProjectToml(flags.DescriptorPath)
			if err != nil {
				return err
			}
...
func parseProjectToml(descriptorPath string) (project.Descriptor, error) {
	const defaultDescriptorPath = "project.toml"

	if descriptorPath == "" {
		if _, err := os.Stat(defaultDescriptorPath); os.IsNotExist(err) {
			return project.Descriptor{}, nil
		}
		return project.ReadProjectDescriptor(defaultDescriptorPath)
	}

	if _, err := os.Stat(descriptorPath); os.IsNotExist(err) {
		return project.Descriptor{}, errors.New(fmt.Sprintf("project descriptor '%s' does not exist", descriptorPath))
	}
	return project.ReadProjectDescriptor(descriptorPath)
}
